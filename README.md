...
├── package-lock.json  # Lock файл для npm ci (ОБЯЗАТЕЛЬНО в репозитории!)
...
- Убедитесь, что `package-lock.json` присутствует в репозитории (он исключен из `.gitignore` для production)
- Репозиторий должен быть публичным или Render должен иметь доступ к приватному репозиторию
- База данных PostgreSQL будет автоматически создана Render на основе `render.yaml`

### Безопасность
- Никогда не коммитите реальные пароли и ключи API
